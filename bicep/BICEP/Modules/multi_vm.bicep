...
// RBAC assignment
resource roleAssignment 'Microsoft.Authorization/roleAssignments@2020-04-01-preview' = [for i in range(0, vmCount):{
  name: guid(vm[i].id, principalId, contributorRoleDefinition.id)
  scope: vm[i]
  dependsOn: [
    vm[i]
  ]
...
resource nic 'Microsoft.Network/networkInterfaces@2021-05-01' = [for i in range(1, vmCount):{
...
output OUTPUT_VM_NAME array = [for i in range(0,vmCount) : {
  name : vm[i].name}]
output OUTPUT_NIC_NAME array = [for i in range(0,vmCount) : {
  name : nic[i].name}]
